  ...
  computed: {
    ...
    validationRules() {
      return !this.$options.validationRules
        ? {}
        : this.$options.validationRules(this.validationRulesVars)
    },
    validationSchema() {
      const fields = Object.entries(this.validationRules).reduce(
        (schema, [field, rules]) => {
          schema[field] = Object.entries(rules).reduce(
    ...
    validationErrors() {
      return Object.keys(this.validationRules).reduce(
        (allErrors, fieldName) => {
          if (!allErrors[fieldName]) allErrors[fieldName] = []
          if (
            !this.$v.fields[fieldName].$dirty ||
            this.$v.fields[fieldName].$pending
          )
            return allErrors
          Object.entries(this.validationRules[fieldName]).forEach(
